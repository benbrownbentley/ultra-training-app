...
// Slide-up toast that fires when the user logs a workout from the Today
// card. Shows "LOGGED · Add actuals →" with the arrow linking into the
// drill-down. Auto-dismisses after 5 seconds; the user can tap the body
// or the close glyph to dismiss sooner.
//
// Owned at the page level via the LoggedToastProvider context so any card
// can fire one with a single hook call.
    ...
    <ToastContext.Provider value={{ show }}>
      ...
      {state && (
        <div
          ...
          aria-live="polite"
        >
          <div className="pointer-events-auto flex max-w-[420px] flex-1 items-center justify-between gap-3 rounded-[12px] border border-emerald-200 bg-white px-3.5 py-2.5 shadow-[0_12px_30px_rgba(16,185,129,0.18)] dark:border-emerald-500/35 dark:bg-[#0f0f11]">
...
export function useLoggedToast() {
