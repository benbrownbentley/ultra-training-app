...
- **Diff should summarise mobility/strength at the title level, not exercise-by-exercise** (UX polish) — the regen diff page currently lists every exercise change inside strength and mobility blocks, while running changes show a short title only. Collapse strength and mobility diff entries to title/high-level summary for consistency. Per-exercise detail belongs on the drill-down, not the diff. Affects `DayDiff` rendering and the `summarisePlannedDetailForDiff` formatter in `lib/preview.ts`.
- **Unsaved-plan warning when leaving the diff** (UX polish) — today, navigating away from the regen result page without clicking Accept silently discards the previewed plan. Add a confirmation modal ("Your new plan won't be saved — leave anyway?") on back-button, tab switch, or any other navigation away. Standard unsaved-changes pattern; cheap insurance against accidental data loss for a flow that already cost an API call.
